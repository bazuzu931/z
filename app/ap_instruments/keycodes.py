...
SYM_LIST = {(k.replace(KL_, '').replace(KD_, '').lower()): v for k, v in locals().items() if k.startswith(KD_) or k.startswith(KL_) }

SYM_LIST['#'] = KEYCODE_POUND
SYM_LIST['@'] = KEYCODE_AT
SYM_LIST['-'] = KEYCODE_MINUS
SYM_LIST[' '] = KEYCODE_SPACE
SYM_LIST['.'] = KEYCODE_NUMPAD_DOT
